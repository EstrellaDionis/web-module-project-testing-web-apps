...
test('renders ONE error message if user enters less then 5 characters into firstname.', async () => {
    ...
    const firstNameField =screen.getByLabelText(/First Name*/i); //this is checking that labelText or InnerText is First Name*
    userEvent.type(firstNameField, 'abc'); // this is typing into our firstNameField: abc 

    const errorMessages = await screen.findAllByTestId('error');
    expect(errorMessages).toHaveLength(1); //this is returning ONE error message and not a specific value
});
...
test('renders all firstName, lastName and email text when submitted. Does NOT render message if message is not submitted.', async () => {
    ...
    userEvent.type(firstNameField, 'dionis');
    userEvent.type(lastNameField, 'estrella');
    userEvent.type(emailField, 'dionis8534@aim.com');
    ...
    await waitFor(() => {
        const firstnameDisplay = screen.queryByText('dionis');
        const lastnameDisplay = screen.queryByText('estrella');
        const emailDisplay = screen.queryByText('dionis8534@aim.com');
